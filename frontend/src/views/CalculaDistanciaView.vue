              ...
              >
                <div class="form-group">
                  ...
                  <input
                    ...
                    v-mask="'#####-###'"
                    :disabled="
                      !urlBrasilApi() || disabledLimiteReqApi() || loading
                    "
  ...
  export default {
    ...
    methods: {
      ...
      async buscarParametroUrlBrasilApi() {
        if (this.urlBrasilApi() != null) return;

        this.loading = true;

        try {
          const response = await this.$axios.get('/parametro/URLBrasilApi');
          localStorage.setItem('urlBrasilApi', response.data.valor);
        } catch (error) {
      ...
      async buscarParametroLimiteReqApi() {
        if (this.limiteRequisicoesApi() != null) return;

        this.loading = true;

        try {
          const response = await this.$axios.get('/parametro/QtdLmtReqAPI');
          localStorage.setItem('lmtReqBrasilApi', response.data.valor);
        } catch (error) {
          console.log('Erro: ', error);
      ...
      // API
      urlBrasilApi() {
        return localStorage.getItem('urlBrasilApi') || null;
      },

      limiteRequisicoesApi() {
        return localStorage.getItem('lmtReqBrasilApi') || null;
      },

      disabledLimiteReqApi() {
        return this.qtdReqApi >= this.limiteRequisicoesApi();
      },
      ...
      async consultaBrasilAPI(cep) {
        try {
          ...
          const { data } = await this.$axios.get(this.urlBrasilApi() + cep);
      ...
      async adicionarDistancia() {
        ...
        try {
          ...
          const distancia = this.calcularDistanciaHaversine(coordenadasOrigem, coordenadasDestino);
          ...
          this.$toasted.show(
            'Distância calculada e inserida com sucesso!',
